...
public class SistemaRegistroUI extends javax.swing.JFrame {
    ...
    private static boolean datosInicialesCargados;
    ...
    private void configurarVentana() {
        ...
        tablaClientes.getSelectionModel().addListSelectionListener(evt -> tablaClientesValueChanged(evt));
        tablaProductos.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        tablaProductos.getSelectionModel().addListSelectionListener(evt -> tablaProductosValueChanged(evt));
        tablaVentas.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        tablaVentas.getSelectionModel().addListSelectionListener(evt -> tablaVentasValueChanged(evt));
    ...
    private void refrescarVentas() {
        ...
        modelo.setRowCount(0);
        for (Venta venta : ventaServicio.getVentas()) {
            modelo.addRow(new Object[]{
                venta.getCodigo(),
    ...
    private void actualizarCamposProducto() {
        ...
        if ("Electronico".equals(tipo)) {
            lblProductoExtra.setText("Tiene garantia");
            txtProductoTalla.setEnabled(false);
        } else if ("Alimento".equals(tipo)) {
            lblProductoExtra.setText("Es perecible");
            txtProductoTalla.setEnabled(false);
        } else {
            lblProductoExtra.setText("En temporada");
    ...
    private void initComponents() {
        ...
        cbProductoTipo.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Electronico", "Alimento", "Ropa" }));
        ...
        panelConsultaAcciones.setLayout(new java.awt.FlowLayout(java.awt.FlowLayout.RIGHT));
        ...
        panelConsultaVentas.add(scrollDetalleVentaSeleccionada, java.awt.BorderLayout.SOUTH);
    ...
    private void btnProductoRegistrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProductoRegistrarActionPerformed
        ...
        try {
            ...
            Producto producto;
            if ("Electronico".equals(tipo)) {
                producto = new ProductoElectronico(codigoProducto, nombre, precioBase, extra);
            } else if ("Alimento".equals(tipo)) {
                producto = new ProductoAlimento(codigoProducto, nombre, precioBase, extra);
            } else {
                String talla = txtProductoTalla.getText().trim();
                if (talla.isEmpty()) {
                    mostrarError("La talla es obligatoria para productos de ropa.");
                    return;
                }
                producto = new ProductoRopa(codigoProducto, nombre, precioBase, talla, extra);
            }
    ...
    private void tablaVentasValueChanged(javax.swing.event.ListSelectionEvent evt) {
        ...
        int codigo = valorCodigo instanceof Number
                ? ((Number) valorCodigo).intValue()
                : Integer.parseInt(valorCodigo.toString());
    ...
    private Producto construirProductoDesdeFormulario() {
        ...
        boolean extra = chkProductoExtra.isSelected();
        if ("Electronico".equals(tipo)) {
            return new ProductoElectronico(codigoProducto, nombre, precioBase, extra);
        }
        if ("Alimento".equals(tipo)) {
            return new ProductoAlimento(codigoProducto, nombre, precioBase, extra);
        }
        String talla = txtProductoTalla.getText().trim();
        if (talla.isEmpty()) {
            throw new IllegalArgumentException("La talla es obligatoria para productos de ropa.");
        }
        return new ProductoRopa(codigoProducto, nombre, precioBase, talla, extra);
    }
    ...
    private void cargarProductoEnFormulario(Producto producto) {
        ...
        txtProductoTalla.setText("");
        chkProductoExtra.setSelected(false);
        if (producto instanceof ProductoElectronico electronico) {
            cbProductoTipo.setSelectedItem("Electronico");
            chkProductoExtra.setSelected(electronico.isGarantia());
        } else if (producto instanceof ProductoAlimento alimento) {
            cbProductoTipo.setSelectedItem("Alimento");
            chkProductoExtra.setSelected(alimento.isPerecible());
        } else if (producto instanceof ProductoRopa ropa) {
            cbProductoTipo.setSelectedItem("Ropa");
            txtProductoTalla.setText(ropa.getTalla());
            chkProductoExtra.setSelected(ropa.isEnTemporada());
        }
        actualizarCamposProducto();
    }
